...
2. Before any gateway stop/restart/update-restart, confirm the Jarvis-owned systemd safety-belt is installed and/or arm an external OpenCode recovery safety-belt. Current preferred live mechanism is `hermes-gateway.service` `ExecStartPre=-/bin/bash /home/jinwang/workspace/jinwang-jarvis/scripts/arm-opencode-gateway-recovery.sh systemd-ExecStartPre-hermes-gateway`, which fail-opens and arms recovery on every systemd start/restart path, even outside Hermes terminal hooks. The hook should launch the OpenCode worker through `systemd-run --user` as a separate transient unit, keeping tmux only as fallback. The recovery worker sleeps 180 seconds, then checks `hermes-gateway.service`, restarts it only if inactive/failed, writes `~/.hermes/recovery/latest-opencode-recovery.report` or current recovery metadata, and reports back to the origin Discord thread when possible. For long update/reinstall flows, 300 seconds is acceptable, but 180 seconds is Jinwang's default requirement.
