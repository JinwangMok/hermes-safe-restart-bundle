...
PATCH_PATH="${ROOT_DIR}/patches/hermes-safe-restart.patch"
...
if git -C "$HERMES_DIR" apply --3way --check "$PATCH_PATH" >/dev/null 2>&1; then
  ...
  echo "Applied hermes-safe-restart patch to $HERMES_DIR"
  exit 0
fi

echo "Patch no longer applies cleanly. Refresh the bundle in $ROOT_DIR instead of editing Hermes directly." >&2
exit 1
